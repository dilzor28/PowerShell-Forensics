...
# Copy every recentItems
Foreach($user in Get-ChildItem($env:homedrive + '\Users\')){
	if ((-not ($user -contains "default")) -and (-not ($user -match "public"))){
        try {
        new-item -ItemType Directory "$export_path\$user\"
			foreach ($file in Get-ChildItem "$env:homedrive\Users\$user\AppData\Roaming\Microsoft\Windows\Recent"){
				Add-Content -Path "$export_path\$user\recents.txt" -Value ("File: $($file) | LastAccessed: $($file.LastAccessTime) | Creation: $($file.CreationTime)")
				}
	    }
        catch {}
        }
...
foreach ($item in get-childitem "$env:homedrive\Windows\Prefetch"){
    add-content -Path "$export_path\prefetch.txt" -Value ("File: $($item) | LastAccessed: $($item.LastAccessTime) | Creation: $($item.CreationTime)")
}

Copy-Item "$env:homedrive\Windows\System32\winevt\Logs\Security.evtx" "$export_path\security.evtx"


New-IsoFile $export_path "$env:homedrive\Triage.iso" 
